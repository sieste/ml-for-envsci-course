...
\item \textbf{Input layer}: layer providing the input features 
\item \textbf{Output layer}: final layer producing the model output(s)
...
\begin{itemize}
\item Low-level frameworks for implementing neural networks.
...
\begin{itemize}
\item High-level API for building NNs, based on TensorFlow or PyTorch.
\item Simple syntax for composing standard layers into complex deep NN architectures.
\item Ideal for learning and prototyping.
...
\textbf{Workflow:}
\begin{itemize}
\item Load data, parse data types
...
\item Train model, tracking validation loss
\item Evaluate model on test data
\item Benchmark against linear regression
\end{itemize}
\end{frame}
...
\begin{minipage}{.4\textwidth}
\begin{center}
\includegraphics[width=\textwidth]{fig/mlp-example-learningcurve.png}
\end{center}
\end{minipage}%
...
\begin{frame}{Examples}
\begin{itemize}
\item Hazard prediction: MORE HERE
\item Image segmentation: MORE HERE
\item Next-time step prediction: MORE HERE
\end{itemize}
\end{frame}

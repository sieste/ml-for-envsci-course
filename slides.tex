...
\end{itemize}
\item A neuron takes inputs \(\fat{x}=(x_1, \dots, x_k)\), calculates a \textbf{linear combination}, and applies a non-linear activation function.
\end{itemize}
\end{frame}
...
\begin{minipage}{.6\textwidth}
\begin{itemize}
\item UCI Air quality data: hourly urban gas concentration readings
\item Ground truth (\texttt{GT}) concentrations: CO (column 3), Non-metanic hydrocarbons NMHC (5), Benzene C6H6 (6), NOx (8), NO2 (10)
\item Sensor readings \texttt{PT08.S1...5} - CO (4), NMHC (7), NOx (9), NO2 (11), O3 (12)
\item Meteorology: Temp. (13), Rel. Humidity (14), Abs. Humidity (15)
\end{itemize}
\end{minipage}%
\begin{minipage}{.4\textwidth}
\begin{center}
\includegraphics[width=\textwidth]{fig/mlp-example-data-1.png}\\
\end{center}
\end{minipage}%
\vfill
\begin{center}
...
\begin{minipage}{.6\textwidth}
\begin{lstlisting}[language=python]
# train model, monitoring training and 
# validation loss
...
\begin{frame}{Convolution}
\begin{itemize}
\item The input data usually have multiple channels (color channels, pressure levels, variables)
...
\begin{frame}{Convolution}
\begin{itemize}
\item By defining multiple convolution kernels, we produce multiple output channels from the same input.
\end{itemize}
\vfill
\begin{center}
...
\end{center}
\begin{itemize}
\item This is the typical setup for a 2d convolution layer in a convolutional neural network.
\item The trainable parameters in a 2d convolution are stored in a 4d data structure with dimensions (width, height, input channels, output channels).
\end{itemize}
\end{frame}
...
\begin{itemize}
\item inputs \(x[i,j,c]\) indexed by space \((i,j)\) and channel \(c=1,\dots,C\)
\item outputs \(y[i,j]\) indexed by space \((i,j)\)
\item kernel \(k[m,n,c]\) of width and height \(m = -w,\dots,+w\) and \(C\) channels
\end{itemize}
\[y[i,j] = \sum_{m = -w}^{w} \sum_{n = -w}^{w} \sum_{c=1}^C k[m,n,c] \cdot x[i+m,j+n,c]\]
\end{frame}


\begin{frame}{Convolution}
\begin{itemize}
\item Convolution layers can be chained to produce a deep convolutional neural network (Deep CNN).
\end{itemize}
\vfill
\begin{center}

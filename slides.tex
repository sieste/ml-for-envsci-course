...
\begin{itemize}
\item High-dimensional (space, time, multivariable)
\item Complex interactions between variables
\item Data-rich but limited understanding of mechanisms
\item Clear framing as input-output relationship
\item Focus on skilful prediction
\end{itemize}
...
\begin{itemize}
\item We have a target (outcome) measurement that we wish to predict. 
...
\begin{frame}{Familiar example: Linear regression}
\begin{itemize}
...
\item The impurity reduction (or "information gain") caused by the split is given by \[IG = G(\fat{y}) - G(\fat{y}_{left}, \fat{y}_{right})\]

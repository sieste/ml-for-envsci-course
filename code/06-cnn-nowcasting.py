...
# "NimrodMLdata-1000x40x40x20-seed1.dat.zip" from there into your data/ directory
...
# plot a sample sequence and corresponding target
i_plt = 40 
fig,axs = plt.subplots(1, 5, figsize=(13,4))
plot_arr = movies[i_plt,:,:,:5]
vmax = np.max(plot_arr)
...
# plot a sample input, target and model output from the test data set
...
# visualise the difference between CNN and U-Net
i_plt = 3 # also good: 27
vmax = np.max([y_test[i_plt,...], pred_test[i_plt,...], pred2_test[i_plt,...]])
fig,ax = plt.subplots(1,3,figsize=(12,3))
ax[0].imshow(pred_test[i_plt,...], vmax=vmax, cmap='Blues')
ax[0].set_title('Deep CNN')
ax[1].imshow(pred2_test[i_plt,...], vmax=vmax, cmap='Blues')
